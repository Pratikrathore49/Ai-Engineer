...
def count_prompt_tokens():
    ...
    TOKENS_FOR_REPLY_PRIMING = 3  # every reply is primed with <|start|>assistant<|message|>
...
def real_cost_tracking():
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "user", "content": "List 5 benefits of Python for AI development."}
        ],
        temperature=0.7
    )
...
def context_window_demo():
    ...
    if total_input_tokens > max_input_tokens:
        print(f"WARNING: Input too long by {total_input_tokens - max_input_tokens:,} tokens!")
        print("Options:")
        ...
        allowed_content_tokens = max_input_tokens - (4 * len(messages)) - 3 - len(enc.encode("Summarize the following document."))
